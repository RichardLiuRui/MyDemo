...
/**
 * Store the configuration & values to shared preference
 * 
 */
public class SharedPreferenceUtil
{
        ...
        try {
            switch (type) {
                ...
                case "Integer":
                    ...
                    break;
                default:
                    String json = GsonUtil.toGson(value);
        ...
        try {
            switch (type) {
                ...
                default:
                    ...
                    if (json != null && !json.equals("") && json.length() > 0) {
                        result = GsonUtil.fromGson(json, defaultValue.getClass());
                    } else {
                        result = defaultValue;
                    }
                    break;
    ...
    public static <T> boolean putListData(String key, List<T> list) {
        ...
        try {
            switch (type) {
                ...
                default:
                    for (int i = 0; i < list.size(); i++) {
                        JsonElement obj = GsonUtil.getGson().toJsonTree(list.get(i));
                        array.add(obj);
                    }
                    break;
            }
    ...
    public static <T> List<T> getListData(String key, Class<T> cls) {
        ...
        if (!json.equals("") && json.length() > 0) {
            ...
            for (JsonElement elem : array) {
                list.add(GsonUtil.fromGson(elem, cls));
            }
        }
        return list;
    }
    ...
    public static <K, V> boolean putHashMapData(String key, Map<K, V> map) {
        ...
        Editor editor = sp.edit();
        try {
            String json = GsonUtil.toGson(map);
            editor.putString(key, json);
            result = true;
        } catch (Exception e) {
            result = false;
    ...
    public static <V> HashMap<String, V> getHashMapData(String key, Class<V> clsV) {
        ...
        for (Map.Entry<String, JsonElement> entry : entrySet) {
            ...
            map.put(entryKey, GsonUtil.fromGson(value, clsV));
        }
        Log.e("SharedPreferencesUtil", obj.toString());
        return map;
    }

    public synchronized static void clearKeyValues(String key)
    {
        Editor editor = sp.edit();
        editor.remove(key);
        editor.apply();
